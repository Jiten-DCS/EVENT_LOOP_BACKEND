...
graph TD
    ...
    B --> D[Manage Vendors];
    D --> DA[View Vendors];
    D --> DB[Approve/Reject Vendor];
    B --> E[Manage Categories];
    E --> EA[Create Category];
    E --> EB[View Categories];
    B --> F[Manage Offers (Platform)];
    F --> FA[View All Offers];
    B --> G[Oversee Bookings];
    G --> GA[View User/Vendor Bookings];
    G --> GB[Update Booking Status];
    B --> H[Manage Reviews];
    H --> HA[Moderate/Delete Reviews];
```
